...
class NavBar2 extends React.Component {
  ...
  render() {
    ...
    // todo: set article url to: href={`${siteMetadata.canonicalUrl}/#articles`}
    return (
      <div>
          ...
          <Container>
            ...
            <Collapse isOpen={this.state.isOpen} navbar>
              <Nav className="ml-auto" navbar>
                      ...
                      {categories.map((category) => {
                        return (
                          <DropdownItem key={category.fieldValue}>
                            <NavLink href={`${siteMetadata.canonicalUrl}/#${category.fieldValue}`}>{_.startCase(category.fieldValue)}</NavLink>
                          </DropdownItem>
                        )
                      })}
                    </DropdownMenu>
                ...
                <NavItem>
                  <NavLink disabled href={`${siteMetadata.canonicalUrl}/#articles`}>Articles</NavLink>
                </NavItem>
                <UncontrolledDropdown nav inNavbar>
                  <DropdownToggle nav caret>
                    About
